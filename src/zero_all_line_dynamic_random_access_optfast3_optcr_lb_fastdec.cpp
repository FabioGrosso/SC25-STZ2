                           ...
                           size_t marker_chunk_dim)
{
    ...
    // skip the intermediate quant-index vector. Symbols decode in Morton chunk
    // order but unpred values must be consumed in row-major order, so qi==0
    // positions are recorded and patched afterwards in sorted (row-major) order.
    const double q_eb = quantizer.get_eb();
    const int q_radius = quantizer.get_out_range().second / 2;
    std::vector<size_t> unpred_pos;
    bool fused = quant_inds_size == num &&
                 encoder.decode_marked_transform(buffer_pos, quant_inds_size, deData,
                     [&](size_t idx, int qi) -> T {
                         if (qi == 0) {
                             unpred_pos.push_back(idx);
                             return 0;
                         }
    ...
    if (fused) {
        if (!unpred_pos.empty()) {
            std::sort(unpred_pos.begin(), unpred_pos.end());
            for (size_t idx : unpred_pos) {
                deData[idx] = quantizer.recover(0, 0);
            }
        }
    } else {
...
int run_typed(int argc, char* argv[])
{
    ...
    // z-plane only reads the reference, so z-slabs are independent.
    {
        ...
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            const int block = tasks[t][0], z0 = tasks[t][1], z1 = tasks[t][2];
            depreprocess_block_zslab(block, deData[block], reconstructed_sub_0, de_sub_block[block],
                                     dim_x, dim_y, dim_z, z0, z1);
        }
    }
