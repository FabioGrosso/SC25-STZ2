    ...
    for (int z = 0; z < dim_z; ++z)
    {
        for (int y = 0; y < dim_y; ++y)
        {
            for (int x = 0; x < dim_x; ++x)
            {
                ...
                switch(block)
                {
                    case 0: // "001" - 1D Cubic in X
                        if (x == dim_x - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (x == dim_x - 2 || x == 0)
                            diff[idx] = sub_block[idx] - (0.5f * ref[idx] + 0.5f * ref[idx + 1]);
                        else
                        {
                            diff[idx] = sub_block[idx] - (W[0]*ref[idx-1] + W[1]*ref[idx] + 
                                                          W[2]*ref[idx+1] + W[3]*ref[idx+2]);
                        }
                        break;

                    case 1: // "010" - 1D Cubic in Y
                        if (y == dim_y - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (y == dim_y - 2 || y == 0)
                            diff[idx] = sub_block[idx] - (0.5f * ref[idx] + 0.5f * ref[idx + dim_x]);
                        else
                        {
                            diff[idx] = sub_block[idx] - (W[0]*ref[idx-dim_x] + W[1]*ref[idx] + 
                                                          W[2]*ref[idx+dim_x] + W[3]*ref[idx+dim_x*2]);
                        }
                        break;

                    case 2: // "011" - 12-point Truncated Bicubic in Y and X
                        if (x == dim_x - 1 || y == dim_y - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (y == dim_y - 2 || y == 0 || x == dim_x - 2 || x == 0)
                            diff[idx] = sub_block[idx] - (0.25f * ref[idx] + 0.25f * ref[idx + dim_x + 1] +
                                                          0.25f * ref[idx + dim_x] + 0.25f * ref[idx + 1]);
                        else
                        {
                            // // --- Original 16-point Bicubic ---
                            // float pred_y = 0.0;
                            // for (int dv = -1; dv <= 2; ++dv) {
                            //     float pred_x = 0.0;
                            //     int base_idx = z * dim_xy + (y + dv) * dim_x + x;
                            //     pred_x += W[0] * ref[base_idx - 1] + W[1] * ref[base_idx] + 
                            //               W[2] * ref[base_idx + 1] + W[3] * ref[base_idx + 2];
                            //     pred_y += W[dv+1] * pred_x;
                            // }
                            // diff[idx] = sub_block[idx] - pred_y;
                            

                            // --- 12-point Truncated Bicubic (No Outer Corners) ---
                            // Core 4 points (Inner X, Inner Y): Weight W[1]*W[1] = 0.5625 * 0.5625
                            float w_core = 0.31640625f;
                            float core_sum = ref[idx] + ref[idx + 1] + ref[idx + dim_x] + ref[idx + dim_x + 1];

                            // Edge 8 points (Outer in one dim, Inner in another): Weight W[0]*W[1] = -0.0625 * 0.5625
                            float w_edge = -0.033203125f;
                            float edge_sum = ref[idx - 1] + ref[idx + 2] + // Y inner (0), X outer (-1, 2)
                                             ref[idx + dim_x - 1] + ref[idx + dim_x + 2] + // Y inner (1), X outer (-1, 2)
                                             ref[idx - dim_x] + ref[idx - dim_x + 1] + // Y outer (-1), X inner (0, 1)
                                             ref[idx + 2 * dim_x] + ref[idx + 2 * dim_x + 1]; // Y outer (2), X inner (0, 1)

                            float pred_y = w_core * core_sum + w_edge * edge_sum;
                            diff[idx] = sub_block[idx] - pred_y;
                        }
                        break;

                    case 3: // "100" - 1D Cubic in Z
                        if (z == dim_z - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (z == dim_z - 2 || z == 0)
                            diff[idx] = sub_block[idx] - (0.5f * ref[idx] + 0.5f * ref[idx + dim_xy]);
                        else
                        {
                            diff[idx] = sub_block[idx] - (W[0]*ref[idx-dim_xy] + W[1]*ref[idx] + 
                                                          W[2]*ref[idx+dim_xy] + W[3]*ref[idx+dim_xy*2]);
                        }
                        break;

                    case 4: // "101" - 12-point Truncated Bicubic in Z and X
                        if (x == dim_x - 1 || z == dim_z - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (z == dim_z - 2 || z == 0 || x == dim_x - 2 || x == 0)
                            diff[idx] = sub_block[idx] - (0.25f * ref[idx] + 0.25f * ref[idx + dim_xy + 1] +
                                                          0.25f * ref[idx + 1] + 0.25f * ref[idx + dim_xy]);
                        else
                        {
                            // // --- Original 16-point Bicubic ---
                            // float pred_z = 0.0;
                            ...
                            //     float pred_x = 0.0;
                            //     int base_idx = (z + dw) * dim_xy + y * dim_x + x;
                            //     pred_x += W[0] * ref[base_idx - 1] + W[1] * ref[base_idx] + 
                            //               W[2] * ref[base_idx + 1] + W[3] * ref[base_idx + 2];
                            //     pred_z += W[dw+1] * pred_x;
                            // }
                            // diff[idx] = sub_block[idx] - pred_z;
                            ...
                            float w_edge = -0.033203125f;
                            float edge_sum = ref[idx - 1] + ref[idx + 2] + // Z inner (0), X outer (-1, 2)
                                             ref[idx + dim_xy - 1] + ref[idx + dim_xy + 2] + // Z inner (1), X outer (-1, 2)
                                             ref[idx - dim_xy] + ref[idx - dim_xy + 1] + // Z outer (-1), X inner (0, 1)
                                             ref[idx + 2 * dim_xy] + ref[idx + 2 * dim_xy + 1]; // Z outer (2), X inner (0, 1)

                            float pred_z = w_core * core_sum + w_edge * edge_sum;
                            diff[idx] = sub_block[idx] - pred_z;
                        }
                        break;

                    case 5: // "110" - 12-point Truncated Bicubic in Z and Y
                        if (z == dim_z - 1 || y == dim_y - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (y == dim_y - 2 || y == 0 || z == dim_z - 2 || z == 0)
                            diff[idx] = sub_block[idx] - (0.25f * ref[idx] + 0.25f * ref[idx + dim_xy + dim_x] +
                                                          0.25f * ref[idx + dim_xy] + 0.25f * ref[idx + dim_x]);
                        else
                        {
                            // // --- Original 16-point Bicubic ---
                            // float pred_z = 0.0;
                            ...
                            //     float pred_y = 0.0;
                            //     for (int dv = -1; dv <= 2; ++dv) {
                            //         int base_idx = (z + dw) * dim_xy + (y + dv) * dim_x + x;
                            //         pred_y += W[dv+1] * ref[base_idx];
                            //     }
                            //     pred_z += W[dw+1] * pred_y;
                            // }
                            // diff[idx] = sub_block[idx] - pred_z;
                            ...
                            float w_edge = -0.033203125f;
                            float edge_sum = ref[idx - dim_x] + ref[idx + 2 * dim_x] + // Z inner (0), Y outer (-1, 2)
                                             ref[idx + dim_xy - dim_x] + ref[idx + dim_xy + 2 * dim_x] + // Z inner (1), Y outer (-1, 2)
                                             ref[idx - dim_xy] + ref[idx - dim_xy + dim_x] + // Z outer (-1), Y inner (0, 1)
                                             ref[idx + 2 * dim_xy] + ref[idx + 2 * dim_xy + dim_x]; // Z outer (2), Y inner (0, 1)
                    ...
                    case 6: // "111" - True 64-point Tricubic in Z, Y, and X
                        if (x == dim_x - 1 || y == dim_y - 1 || z == dim_z - 1)
                            diff[idx] = sub_block[idx] - ref[idx];
                        else if (z == dim_z - 2 || z == 0 || y == dim_y - 2 || y == 0 || x == dim_x - 2 || x == 0)
                        {
                            int idx_z1 = idx + dim_xy; int idx_y1 = idx + dim_x; int idx_x1 = idx + 1;
                            int idx_xy1 = idx_y1 + 1;  int idx_zy1 = idx_z1 + dim_x; int idx_zx1 = idx_z1 + 1;
                ...
                switch(block)
                {
                    case 0:
                        if (x == dim_x - 1)
                            ...
                            de_sub[idx] = deData[idx] + (0.5f * ref[idx] + 0.5f * ref[idx + 1]);
                        else
                            de_sub[idx] = deData[idx] + (W[0]*ref[idx-1] + W[1]*ref[idx] + 
                                                         W[2]*ref[idx+1] + W[3]*ref[idx+2]);
                        break;
                    ...
                    case 1:
                        if (y == dim_y - 1)
                            ...
                            de_sub[idx] = deData[idx] + (0.5f * ref[idx] + 0.5f * ref[idx + dim_x]);
                        else
                            de_sub[idx] = deData[idx] + (W[0]*ref[idx-dim_x] + W[1]*ref[idx] + 
                                                         W[2]*ref[idx+dim_x] + W[3]*ref[idx+dim_x*2]);
                        break;

                    case 2: // 12-point Truncated Bicubic
                        if (x == dim_x - 1 || y == dim_y - 1)
                            de_sub[idx] = deData[idx] + ref[idx];
                        else if (y == dim_y - 2 || y == 0 || x == dim_x - 2 || x == 0)
                            de_sub[idx] = deData[idx] + (0.25f * ref[idx] + 0.25f * ref[idx + dim_x + 1] +
                                                         0.25f * ref[idx + dim_x] + 0.25f * ref[idx + 1]);
                        else
                        {
                            // // --- Original 16-point Bicubic ---
                    ...
                    case 3:
                        if (z == dim_z - 1)
                            ...
                            de_sub[idx] = deData[idx] + (0.5f * ref[idx] + 0.5f * ref[idx + dim_xy]);
                        else
                            de_sub[idx] = deData[idx] + (W[0]*ref[idx-dim_xy] + W[1]*ref[idx] + 
                                                         W[2]*ref[idx+dim_xy] + W[3]*ref[idx+dim_xy*2]);
                        break;

                    case 4: // 12-point Truncated Bicubic
                        if (x == dim_x - 1 || z == dim_z - 1)
                            de_sub[idx] = deData[idx] + ref[idx];
                        else if (z == dim_z - 2 || z == 0 || x == dim_x - 2 || x == 0)
                            de_sub[idx] = deData[idx] + (0.25f * ref[idx] + 0.25f * ref[idx + dim_xy + 1] +
                                                         0.25f * ref[idx + 1] + 0.25f * ref[idx + dim_xy]);
                        else
                        {
                            // // --- Original 16-point Bicubic ---
                    ...
                    case 5: // 12-point Truncated Bicubic
                        if (z == dim_z - 1 || y == dim_y - 1)
                            de_sub[idx] = deData[idx] + ref[idx];
                        else if (y == dim_y - 2 || y == 0 || z == dim_z - 2 || z == 0)
                            de_sub[idx] = deData[idx] + (0.25f * ref[idx] + 0.25f * ref[idx + dim_xy + dim_x] +
                                                         0.25f * ref[idx + dim_xy] + 0.25f * ref[idx + dim_x]);
                        else
                        {
                            // // --- Original 16-point Bicubic ---
