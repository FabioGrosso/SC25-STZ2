...
int run_typed(int argc, char* argv[])
{
    ...
    for (int t = 0; t < static_cast<int>(low_zslab_tasks.size()); ++t) {
        ...
        preprocess_block_fast(block, low_block_data[block + 1], low_block_data[0], low_diff_data[block],
                              low_dim_x, low_dim_y, low_dim_z, -1, -1, z1, 0, 0, z0);
    }
    ...
    for (int t = 0; t < static_cast<int>(low_zslab_tasks.size()); ++t) {
        const int block = low_zslab_tasks[t][0], z0 = low_zslab_tasks[t][1], z1 = low_zslab_tasks[t][2];
        depreprocess_block_fast(block, low_diff_data[block], low_block_data[0], low_de_sub_block[block],
                                low_dim_x, low_dim_y, low_dim_z, -1, -1, z1, 0, 0, z0);
    }
    ...
    // parallel-for over blocks idles cores on its tail.
    {
        ...
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            ...
            preprocess_block_fast(block, sub_block_data[block + 1], reconstructed_sub_0, diff_data[block],
                                  dim_x, dim_y, dim_z, -1, -1, z1, 0, 0, z0);
        }
    }
    ...
    //           << low_time_taken_decompress_sz << " sec" << std::endl;
    {
        ...
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            const int block = tasks[t][0], z0 = tasks[t][1], z1 = tasks[t][2];
            depreprocess_block_fast(block, low_deData[block], decompressed_data, low_de_sub_block[block],
                                    low_dim_x, low_dim_y, low_dim_z, -1, -1, z1, 0, 0, z0);
        }
    }
    ...
    // z-plane only reads the reference, so z-slabs are independent.
    {
        ...
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            const int block = tasks[t][0], z0 = tasks[t][1], z1 = tasks[t][2];
            depreprocess_block_fast(block, deData[block], reconstructed_sub_0, de_sub_block[block],
                                    dim_x, dim_y, dim_z, -1, -1, z1, 0, 0, z0);
        }
    }
