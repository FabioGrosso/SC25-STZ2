...
int run_typed(int argc, char* argv[])
{
    ...
    // Phase 1: preprocess all 7 blocks, z-slab flattened across cores. Block 6's
    // preprocess alone costs as much as the other six combined, so a plain
    // parallel-for over blocks idles cores on its tail.
    {
        constexpr int kZChunk = 16;
        std::vector<std::array<int, 3>> tasks;
        tasks.reserve(7 * ((dim_z + kZChunk - 1) / kZChunk));
        for (int block = 0; block < 7; ++block)
        ...
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            ...
            preprocess_block_zslab(block, sub_block_data[block + 1], reconstructed_sub_0, diff_data[block],
                                   dim_x, dim_y, dim_z, z0, z1);
        }
    }
    // Phase 2: SZ-encode each block (already balanced ~0.24s each across blocks).
    #pragma omp parallel for reduction(+:allSize)
    for (int block = 0; block < 7; ++block)
    {
        comp[block] = SZ_compress(diff_data[block], dim_x, dim_y, dim_z,
                                  roi_high_dim, mult_high * eb, compressedSize[block],
                                  /*writeDec=*/false);
