...
// Each output row interleaves exactly two source rows (x parity), so hoist the
// parity selection out of the x loop; the pair-interleave inner loop vectorizes.
template <typename T>
void merge_sub_blocks_to_full(T* sub_blocks[8], T* full_data, int sub_dim_x, int sub_dim_y, int sub_dim_z) {
    ...
    for (int z = 0; z < full_z; ++z) {
        const size_t sub_plane = static_cast<size_t>(sub_dim_y) * sub_dim_x;
        const size_t row_base_z = static_cast<size_t>(z / 2) * sub_plane;
        const int zbit = (z & 1) << 2;
        for (int y = 0; y < full_y; ++y) {
            const size_t row = row_base_z + static_cast<size_t>(y / 2) * sub_dim_x;
            const T* s0 = sub_blocks[zbit | ((y & 1) << 1)] + row;
            const T* s1 = sub_blocks[zbit | ((y & 1) << 1) | 1] + row;
            T* dst = full_data + static_cast<size_t>(z) * (full_y * full_x) + static_cast<size_t>(y) * full_x;
            for (int sx = 0; sx < sub_dim_x; ++sx) {
                dst[2 * sx] = s0[sx];
                dst[2 * sx + 1] = s1[sx];
            }
        }
...
// Slice only the z-planes of one parity. Even z feeds sub-blocks 0..3, odd z
// feeds 4..7, so the parity-0 pass alone completes the base-block input.
template <typename T>
void slice_full_data_parity(const T* full_data, T* sub_block_data[8], int dim_x, int dim_y, int dim_z, int z_parity) {
    #pragma omp parallel for
    for (int zz = 0; zz < dim_z / 2; ++zz) {
        const int z = 2 * zz + z_parity;
        for (int y = 0; y < dim_y; ++y) {
            for (int x = 0; x < dim_x; ++x) {
                int sub_index = ((z & 1) << 2) | ((y & 1) << 1) | (x & 1);
                int pos = (z / 2) * (dim_y / 2 * dim_x / 2) + (y / 2) * (dim_x / 2) + x / 2;
                sub_block_data[sub_index][pos] = full_data[(size_t)z * (dim_x * dim_y) + (size_t)y * dim_x + x];
            }
        }
    }
}
...
// Fused prediction+reliability of one cubic line, sharing the second
// differences between the two:  pred = (-a+9b+9c-d)/16 == (b+c)/2 - (c0+c1)/16
// (algebraically identical, ~25% fewer ops; float rounding differs in the last
// ulp, which shifts the quantized stream microscopically -- both codec sides
// share this code, so reconstruction stays exact and error-bounded).
// Disable with -DPRED_NO_CSE to reproduce fastdec4's bytes.
template <typename T>
static inline void cubic_line_pv_cse(T a, T b, T c, T d, T& p, T& v)
{
    constexpr T eps = 1e-12f;
    const T c0 = a - 2.0f * b + c;
    const T c1 = b - 2.0f * c + d;
    v = 0.5f * (c0 * c0 + c1 * c1) + eps;
#ifdef PRED_NO_CSE
    p = cubic_line_pred(a, b, c, d);
#else
    p = 0.5f * (b + c) - (1.0f / 16.0f) * (c0 + c1);
#endif
}

// PRED_WPOW2: sharper inverse-variance weighting (w ~ 1/v^2 instead of 1/v).
template <typename T>
...
template <typename T>
static inline void cubic_line_pv(T a, T b, T c, T d, T& p, T& v)
{
    p = cubic_line_pred(a, b, c, d);
    v = cubic_line_var(a, b, c, d);
}

// Dynamic prediction for the plane blocks (target offset along two axes with
// strides s1/s2): the two diagonal cubic lines, optionally augmented with two
// axis-pair candidates (cubic along one axis, averaged across the other).
template <typename T>
static inline T predict_plane_dyn(const T* ref, int idx, int s1, int s2)
{
    T p0, v0, p1, v1;
    cubic_line_pv(ref[idx - s1 - s2], ref[idx], ref[idx + s1 + s2],
                  ref[idx + 2 * s1 + 2 * s2], p0, v0);
    cubic_line_pv(ref[idx + 2 * s1 - s2], ref[idx + s1], ref[idx + s2],
                  ref[idx - s1 + 2 * s2], p1, v1);
#ifndef PRED_PLANE_AXIS
    return blend2(p0, v0, p1, v1);
#else
    T pa0, va0, pa1, va1;
    cubic_line_pv(ref[idx - s1], ref[idx], ref[idx + s1], ref[idx + 2 * s1], pa0, va0);
    cubic_line_pv(ref[idx + s2 - s1], ref[idx + s2], ref[idx + s2 + s1],
                  ref[idx + s2 + 2 * s1], pa1, va1);
    const T p2 = T(0.5f) * (pa0 + pa1);
    const T v2 = T(0.5f) * (va0 + va1);
    T pb0, vb0, pb1, vb1;
    cubic_line_pv(ref[idx - s2], ref[idx], ref[idx + s2], ref[idx + 2 * s2], pb0, vb0);
    cubic_line_pv(ref[idx + s1 - s2], ref[idx + s1], ref[idx + s1 + s2],
                  ref[idx + s1 + 2 * s2], pb1, vb1);
    const T p3 = T(0.5f) * (pb0 + pb1);
    const T v3 = T(0.5f) * (vb0 + vb1);
    const T w0 = T(1) / v0, w1 = T(1) / v1, w2 = T(1) / v2, w3 = T(1) / v3;
    return (w0 * p0 + w1 * p1 + w2 * p2 + w3 * p3) / (w0 + w1 + w2 + w3);
#endif
}

// Cubic along stride s, averaged over the 4 lines at offsets {0,t1,t2,t1+t2}
// (bilinear across the other two axes). Optional extra candidate for block 6.
template <typename T>
static inline T predict_cube_axis_pair(const T* ref, int idx, int s, int t1, int t2, T& v)
{
    T psum = 0, vsum = 0;
    const int offs[4] = {0, t1, t2, t1 + t2};
    for (int k = 0; k < 4; ++k) {
        const int o = idx + offs[k];
        T p, vv;
        cubic_line_pv(ref[o - s], ref[o], ref[o + s], ref[o + 2 * s], p, vv);
        psum += p;
        vsum += vv;
    }
    v = T(0.25f) * vsum;
...
{
#ifdef PRED_PLANE_AXIS
    if constexpr (Block == 2) {
        return predict_plane_dyn(ref, idx, 1, dim_x);
    } else if constexpr (Block == 4) {
        return predict_plane_dyn(ref, idx, 1, dim_xy);
    } else if constexpr (Block == 5) {
        return predict_plane_dyn(ref, idx, dim_x, dim_xy);
    } else
#else
    if constexpr (Block == 2) {
        T p0, v0, p1, v1;
        cubic_line_pv_cse(ref[idx - dim_x - 1], ref[idx], ref[idx + dim_x + 1],
                          ref[idx + 2 * dim_x + 2], p0, v0);
        cubic_line_pv_cse(ref[idx - dim_x + 2], ref[idx + 1], ref[idx + dim_x],
                          ref[idx + 2 * dim_x - 1], p1, v1);
        return blend2(p0, v0, p1, v1);
    } else if constexpr (Block == 4) {
        T p0, v0, p1, v1;
        cubic_line_pv_cse(ref[idx - dim_xy - 1], ref[idx], ref[idx + dim_xy + 1],
                          ref[idx + 2 * dim_xy + 2], p0, v0);
        cubic_line_pv_cse(ref[idx - dim_xy + 2], ref[idx + 1], ref[idx + dim_xy],
                          ref[idx + 2 * dim_xy - 1], p1, v1);
        return blend2(p0, v0, p1, v1);
    } else if constexpr (Block == 5) {
        T p0, v0, p1, v1;
        cubic_line_pv_cse(ref[idx - dim_xy - dim_x], ref[idx],
                          ref[idx + dim_xy + dim_x],
                          ref[idx + 2 * dim_xy + 2 * dim_x], p0, v0);
        cubic_line_pv_cse(ref[idx - dim_xy + 2 * dim_x], ref[idx + dim_x],
                          ref[idx + dim_xy],
                          ref[idx + 2 * dim_xy - dim_x], p1, v1);
        return blend2(p0, v0, p1, v1);
    } else
#endif
    {
        T p0, v0, p1, v1, p2, v2, p3, v3;
        cubic_line_pv_cse(ref[idx - dim_xy - dim_x - 1], ref[idx],
                          ref[idx + dim_xy + dim_x + 1],
                          ref[idx + 2 * dim_xy + 2 * dim_x + 2], p0, v0);
        cubic_line_pv_cse(ref[idx - dim_xy - dim_x + 2], ref[idx + 1],
                          ref[idx + dim_xy + dim_x],
                          ref[idx + 2 * dim_xy + 2 * dim_x - 1], p1, v1);
        cubic_line_pv_cse(ref[idx - dim_xy + 2 * dim_x - 1], ref[idx + dim_x],
                          ref[idx + dim_xy + 1],
                          ref[idx + 2 * dim_xy - dim_x + 2], p2, v2);
        cubic_line_pv_cse(ref[idx + 2 * dim_xy - dim_x - 1], ref[idx + dim_xy],
                          ref[idx + dim_x + 1],
                          ref[idx - dim_xy + 2 * dim_x + 2], p3, v3);
#ifndef PRED_CUBE_AXIS
        return blend4(p0, v0, p1, v1, p2, v2, p3, v3);
#else
        T v4, v5, v6;
        const T p4 = predict_cube_axis_pair(ref, idx, 1, dim_x, dim_xy, v4);
        const T p5 = predict_cube_axis_pair(ref, idx, dim_x, 1, dim_xy, v5);
        const T p6 = predict_cube_axis_pair(ref, idx, dim_xy, 1, dim_x, v6);
        const T w0 = T(1) / v0, w1 = T(1) / v1, w2 = T(1) / v2, w3 = T(1) / v3;
        const T w4 = T(1) / v4, w5 = T(1) / v5, w6 = T(1) / v6;
        return (w0 * p0 + w1 * p1 + w2 * p2 + w3 * p3 + w4 * p4 + w5 * p5 + w6 * p6) /
...
{
    switch (block) {
        case 0: preprocess_block_const<0>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
        case 1: preprocess_block_const<1>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
        case 2: preprocess_block_const<2>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
        case 3: preprocess_block_const<3>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
        case 4: preprocess_block_const<4>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
        case 5: preprocess_block_const<5>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
        case 6: preprocess_block_const<6>(sub_block, ref, diff, dim_x, dim_y, dim_z, roi_x, roi_y, roi_z, start_x, start_y, start_z); break;
...
// z-slab variants of the split kernels for the flattened (block, z-slab) task
// loops: boundary rows take the branchy generic path, the interior takes the
// branch-free vectorizable path. The plain depreprocess_block_const used by the
// slab loops before kept the per-point boundary branches in the hot loop.
template <int Block, typename T>
void preprocess_block_split_zslab_const(const T* __restrict__ sub_block, const T* __restrict__ ref, T* __restrict__ diff,
                                        int dim_x, int dim_y, int dim_z, int z_begin, int z_end)
{
    if (dim_x < 4 || dim_y < 4 || dim_z < 4) {
        preprocess_block_const<Block>(sub_block, ref, diff, dim_x, dim_y, dim_z,
                                      -1, -1, z_end, 0, 0, z_begin);
        return;
    }
    const int dim_xy = dim_x * dim_y;
    auto process_generic = [&](int x0, int x1, int y0, int y1, int z0, int z1) {
        z0 = std::max(z0, z_begin);
        z1 = std::min(z1, z_end);
        for (int z = z0; z < z1; ++z) {
            for (int y = y0; y < y1; ++y) {
                int idx = z * dim_xy + y * dim_x + x0;
...
template <int Block, typename T>
void depreprocess_block_split_zslab_const(const T* __restrict__ deData, const T* __restrict__ ref, T* __restrict__ de_sub,
                                          int dim_x, int dim_y, int dim_z, int z_begin, int z_end)
{
    if (dim_x < 4 || dim_y < 4 || dim_z < 4) {
        depreprocess_block_const<Block>(deData, ref, de_sub, dim_x, dim_y, dim_z,
...
// Fused depreprocess+merge: reconstruct the sub-block values and scatter them
// straight into the full-resolution volume at this block's parity offsets,
// skipping the intermediate de_sub buffer and the separate merge pass.
template <int Block, typename T>
void depreprocess_block_zslab_to_full_const(const T* __restrict__ deData, const T* __restrict__ ref, T* __restrict__ full_out,
                                            int dim_x, int dim_y, int dim_z, int z_begin, int z_end)
{
    constexpr int sub_index = Block + 1;
    constexpr int px = sub_index & 1;
    constexpr int py = (sub_index >> 1) & 1;
    constexpr int pz = (sub_index >> 2) & 1;
    const int dim_xy = dim_x * dim_y;
    const size_t fdx = 2 * static_cast<size_t>(dim_x);
    const size_t fdxy = fdx * 2 * static_cast<size_t>(dim_y);
    auto emit_range = [&](int x0, int x1, int y0, int y1, int z0, int z1, auto&& predict) {
        z0 = std::max(z0, z_begin);
        z1 = std::min(z1, z_end);
        for (int z = z0; z < z1; ++z) {
            for (int y = y0; y < y1; ++y) {
                int idx = z * dim_xy + y * dim_x + x0;
                T* frow = full_out + (2 * static_cast<size_t>(z) + pz) * fdxy +
                          (2 * static_cast<size_t>(y) + py) * fdx + px;
                for (int x = x0; x < x1; ++x, ++idx) {
                    frow[2 * static_cast<size_t>(x)] = deData[idx] + predict(idx, x, y, z);
                }
            }
        }
    };
    if (dim_x < 4 || dim_y < 4 || dim_z < 4) {
        emit_range(0, dim_x, 0, dim_y, 0, dim_z, [&](int idx, int x, int y, int z) {
            return predict_block_const<Block>(ref, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy);
        });
        return;
    }
    block_boundary_ranges<Block, T>(dim_x, dim_y, dim_z,
        [&](int x0, int x1, int y0, int y1, int z0, int z1) {
            emit_range(x0, x1, y0, y1, z0, z1, [&](int idx, int x, int y, int z) {
                return predict_block_const<Block>(ref, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy);
            });
        });
    ...
    if constexpr (Block == 1 || Block == 2 || Block == 5 || Block == 6) { y0 = 1; y1 = dim_y - 2; }
    if constexpr (Block == 3 || Block == 4 || Block == 5 || Block == 6) { z0 = 1; z1 = dim_z - 2; }
    emit_range(x0, x1, y0, y1, z0, z1, [&](int idx, int, int, int) {
        return predict_interior_const<Block>(ref, idx, dim_x, dim_xy);
    });
}

template <typename T>
void depreprocess_block_zslab_to_full(int block, const T* deData, const T* ref, T* full_out,
                                      int dim_x, int dim_y, int dim_z, int z_begin, int z_end)
{
    switch (block) {
        case 0: depreprocess_block_zslab_to_full_const<0>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 1: depreprocess_block_zslab_to_full_const<1>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 2: depreprocess_block_zslab_to_full_const<2>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 3: depreprocess_block_zslab_to_full_const<3>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 4: depreprocess_block_zslab_to_full_const<4>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 5: depreprocess_block_zslab_to_full_const<5>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 6: depreprocess_block_zslab_to_full_const<6>(deData, ref, full_out, dim_x, dim_y, dim_z, z_begin, z_end); break;
        default: std::cerr << "Unsupported block index in depreprocess_block_zslab_to_full\n"; break;
    }
}

template <typename T>
void preprocess_block_zslab(int block, const T* sub_block, const T* ref, T* diff,
                            int dim_x, int dim_y, int dim_z, int z_begin, int z_end)
{
    switch (block) {
        case 0: preprocess_block_split_zslab_const<0>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 1: preprocess_block_split_zslab_const<1>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 2: preprocess_block_split_zslab_const<2>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 3: preprocess_block_split_zslab_const<3>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 4: preprocess_block_split_zslab_const<4>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 5: preprocess_block_split_zslab_const<5>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 6: preprocess_block_split_zslab_const<6>(sub_block, ref, diff, dim_x, dim_y, dim_z, z_begin, z_end); break;
        default: std::cerr << "Unsupported block index in preprocess_block_zslab\n"; break;
    }
}

template <typename T>
void depreprocess_block_zslab(int block, const T* deData, const T* ref, T* de_sub,
                              int dim_x, int dim_y, int dim_z, int z_begin, int z_end)
{
    switch (block) {
        case 0: depreprocess_block_split_zslab_const<0>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 1: depreprocess_block_split_zslab_const<1>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 2: depreprocess_block_split_zslab_const<2>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 3: depreprocess_block_split_zslab_const<3>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 4: depreprocess_block_split_zslab_const<4>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 5: depreprocess_block_split_zslab_const<5>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        case 6: depreprocess_block_split_zslab_const<6>(deData, ref, de_sub, dim_x, dim_y, dim_z, z_begin, z_end); break;
        default: std::cerr << "Unsupported block index in depreprocess_block_zslab\n"; break;
    }
}

#define preprocess_block preprocess_block_split
// Full-volume calls dispatch to the branchless split kernel; ROI/query calls
// (roi_*/start_* set) fall back to depreprocess_block_fast inside the split driver.
#define depreprocess_block depreprocess_block_split

//---------------------------------------------------------------------
// SZ compression/decompression and file I/O routines
//---------------------------------------------------------------------
// Quantize n contiguous values without writing decompressed values back,
// bit-exact with LinearQuantizer::quantize_and_overwrite(x, 0); unpredictable
// values are collected into a caller-owned vector (kept in element order).
template <typename T>
void quantize_range_nodec(const T* data, size_t n, double eb_d, int radius,
                          const T* dlut, int* qout, std::vector<T>& unpred)
{
    const double recip = 1.0 / eb_d;
    const double range_lim = static_cast<double>(2 * radius - 1);
    constexpr size_t BS = 1024;
    int shifted[BS];
    for (size_t base = 0; base < n; base += BS) {
        const size_t m = std::min(BS, n - base);
        int bad = 0;
        #pragma omp simd reduction(|:bad)
        for (size_t k = 0; k < m; ++k) {
            const T d = data[base + k];
            const double ad = std::fabs(static_cast<double>(d)) * recip;
            const bool in_range = ad < range_lim;
            const double adc = in_range ? ad : 0.0;
            const int half = (static_cast<int>(adc) + 1) >> 1;
            const T dec_mag = dlut[half];
            const T dec_k = (d < 0) ? -dec_mag : dec_mag;
            const T derr = dec_k - d;
            const bool ok = in_range &&
                            (std::fabs(static_cast<double>(derr)) <= eb_d);
            shifted[k] = (d < 0) ? radius - half : radius + half;
            bad |= ok ? 0 : 1;
        }
        if (!bad) {
            for (size_t k = 0; k < m; ++k) {
                qout[base + k] = shifted[k];
            }
        } else {
            for (size_t k = 0; k < m; ++k) {
                const T d = data[base + k];
                const int64_t q64 =
                    static_cast<int64_t>(std::fabs(static_cast<double>(d)) * recip) + 1;
                int qi = 0;
                if (q64 < 2 * static_cast<int64_t>(radius)) {
                    const int half = static_cast<int>(q64 >> 1);
                    const int qsigned = (d < 0) ? -2 * half : 2 * half;
                    const T dec_k = static_cast<T>(qsigned * eb_d);
                    if (std::fabs(static_cast<double>(dec_k - d)) <= eb_d) {
                        qi = (d < 0) ? radius - half : radius + half;
                    }
                }
                if (qi == 0) unpred.push_back(d);
                qout[base + k] = qi;
            }
        }
    }
...
// Assemble a NOPRED stream from already-quantized indices + collected unpred
// values (the quantizer section is written by hand in LinearQuantizer::save's
// exact byte layout).
template <typename T>
char* SZ_compress_finish(const int* quant_inds, size_t num,
                         const std::vector<T>& unpred,
                         ...
                         size_t marker_chunk_dim, double eb, size_t& outSize)
{
    ...
    encoder.preprocess_encode_fast(quant_inds, num, conf.quantbinCnt);

    size_t bufferSize = std::max<size_t>(
        1000, 1.2 * (unpred.size() * sizeof(T) + 64 + encoder.size_est() + sizeof(int) * num));
    SZ3::uchar* buffer = static_cast<SZ3::uchar*>(malloc(bufferSize));
    SZ3::uchar* buffer_pos = buffer;
    // LinearQuantizer::save byte layout
    buffer_pos[0] = 0b00000010;
    buffer_pos += 1;
    *reinterpret_cast<double*>(buffer_pos) = eb;
    buffer_pos += sizeof(double);
    *reinterpret_cast<int*>(buffer_pos) = conf.quantbinCnt / 2;
    ...
    *reinterpret_cast<size_t*>(buffer_pos) = unpred.size();
    ...
    SZ3::write<size_t>(num, buffer_pos);
    encoder.encode_marked_fast(quant_inds, num, buffer_pos);
    encoder.postprocess_encode();

    const size_t cmpCap = conf.size_est() + ZSTD_compressBound(num * sizeof(T));
    char* cmpData = new char[cmpCap];
    SZ3::uchar* cmp_pos = reinterpret_cast<SZ3::uchar*>(cmpData) + conf.size_est();
    SZ3::Lossless_zstd lossless;
    const size_t cmpDataLen = lossless.compress(buffer, buffer_pos - buffer, cmp_pos,
                                                cmpCap - conf.size_est());
    free(buffer);
    SZ3::uchar* conf_pos = reinterpret_cast<SZ3::uchar*>(cmpData);
    conf.save(conf_pos);
    outSize = conf.size_est() + cmpDataLen;
    SZ3::HuffmanEncoder<int>::clear_forced_marker();
    return cmpData;
}

// Manual ALGO_NOPRED compress for the residual streams (same stream layout the
// library path writes): quantize -> Huffman(table+marked encode) -> zstd. This
// mirrors SZGenericCompressor::compress so each phase can be timed (CMP_PROF2)
// and accelerated. Like the library path, it overwrites oriData with the
// decompressed values (the caller relies on that).
template <typename T>
char* SZ_compress(T* oriData, size_t blksize_x, size_t blksize_y, size_t blksize_z,
                  size_t marker_chunk_dim, double eb, size_t& outSize, bool writeDec = true)
{
    SZ3::HuffmanEncoder<int>::set_forced_marker(blksize_x, blksize_y, blksize_z, marker_chunk_dim);
    SZ3::Config conf(blksize_z, blksize_y, blksize_x);
    conf.cmprAlgo = SZ3::ALGO_NOPRED;
    ...
    if (std::getenv("USE_LIB_COMPRESS")) {  // A/B reference: original library path
        char* compressedData = SZ_compress<T>(conf, oriData, outSize);
        SZ3::HuffmanEncoder<int>::clear_forced_marker();
        return compressedData;
    }

    const bool prof = std::getenv("CMP_PROF2") != nullptr;
    const size_t num = conf.num;
    auto t0 = std::chrono::high_resolution_clock::now();

    SZ3::LinearQuantizer<T> quantizer(conf.absErrorBound, conf.quantbinCnt / 2);
    std::unique_ptr<int[]> quant_inds(new int[num]);  // uninitialized on purpose
    auto t0a = std::chrono::high_resolution_clock::now();
    // Vector-friendly quantization, bit-exact with LinearQuantizer::
    // quantize_and_overwrite(data, 0): same double-precision operations in the
    // same order. Blocks where every value quantizes cleanly (the common case)
    // are emitted branch-free; a block with any out-of-range/validation failure
    // is redone through the real quantizer so the unpred list stays in
    // row-major order. The in-range test is done on the double before the
    // int conversion (ad < 2*radius-1 <=> (int64)ad+1 < 2*radius), so the
    // narrow int32 conversion below cannot overflow on the taken path.
    {
        const double eb_d = conf.absErrorBound;
        const double recip = 1.0 / eb_d;          // == error_bound_reciprocal
        const int radius = conf.quantbinCnt / 2;  // == quantizer radius
        const double range_lim = static_cast<double>(2 * radius - 1);
        // dec value table: dec(half, sign) = (T)(±2*half * eb_d); negation of a
        // float is exact, so one table of the positive branch is bit-identical
        // to the inline int->double multiply chain.
        std::vector<T> dec_lut(static_cast<size_t>(radius) + 1);
        for (size_t h = 0; h < dec_lut.size(); ++h) {
            dec_lut[h] = static_cast<T>(2 * static_cast<int>(h) * eb_d);
        }
        const T* dlut = dec_lut.data();
        constexpr size_t BS = 1024;
        T dec[BS];
        int shifted[BS];
        for (size_t base = 0; base < num; base += BS) {
            const size_t n = std::min(BS, num - base);
            int bad = 0;
            #pragma omp simd reduction(|:bad)
            for (size_t k = 0; k < n; ++k) {
                const T d = oriData[base + k];
                const double ad = std::fabs(static_cast<double>(d)) * recip;
                const bool in_range = ad < range_lim;
                const double adc = in_range ? ad : 0.0;          // keep the cast in int32 range
                const int half = (static_cast<int>(adc) + 1) >> 1;
                const T dec_mag = dlut[half];
                const T dec_k = (d < 0) ? -dec_mag : dec_mag;
                const T derr = dec_k - d;                        // float subtract, as in the lib
                const bool ok = in_range &&
                                (std::fabs(static_cast<double>(derr)) <= eb_d);
                dec[k] = dec_k;
                shifted[k] = (d < 0) ? radius - half : radius + half;
            ...
            if (!bad) {
                if (writeDec) {
                    for (size_t k = 0; k < n; ++k) {
                        oriData[base + k] = dec[k];
                        quant_inds[base + k] = shifted[k];
                    }
                } else {  // high-level streams never reuse the decompressed diff
                    for (size_t k = 0; k < n; ++k) {
                        quant_inds[base + k] = shifted[k];
                    }
                }
                ...
                for (size_t k = 0; k < n; ++k) {
                    quant_inds[base + k] = quantizer.quantize_and_overwrite(oriData[base + k], 0);
                }
            }
        }
    }
    ...
    SZ3::HuffmanEncoder<int> encoder;
    encoder.preprocess_encode_fast(quant_inds.get(), num, quantizer.get_out_range().second);
    auto t2 = std::chrono::high_resolution_clock::now();

    size_t bufferSize = std::max<size_t>(
        1000, 1.2 * (quantizer.size_est() + encoder.size_est() + sizeof(int) * num));
    SZ3::uchar* buffer = static_cast<SZ3::uchar*>(malloc(bufferSize));
    SZ3::uchar* buffer_pos = buffer;
    quantizer.save(buffer_pos);
    encoder.save(buffer_pos);
    SZ3::write<size_t>(num, buffer_pos);
    encoder.encode_marked_fast(quant_inds.get(), num, buffer_pos);
    encoder.postprocess_encode();
    ...
    if (prof) {
        auto ms = [](auto a, auto b) {
            return std::chrono::duration_cast<std::chrono::microseconds>(b - a).count() * 1e-3;
        };
        fprintf(stderr, "[cmp prof %zu^3] alloc %.1f ms, quantize %.1f ms, hufftree %.1f ms, encode %.1f ms, zstd %.1f ms\n",
                blksize_x, ms(t0, t0a), ms(t0a, t1), ms(t1, t2), ms(t2, t3), ms(t3, t4));
    }
    SZ3::HuffmanEncoder<int>::clear_forced_marker();
    return cmpData;
}
...
// Manual ALGO_NOPRED decompress (same stream layout the SZ_decompress library
// path reads) so the Huffman stage can use the table-based decode_fast instead
// of the per-bit tree walk. Falls back to the library path if the stream is not
// the expected NOPRED layout.
template <typename T>
...
{
    SZ3::HuffmanEncoder<int>::set_forced_marker(blksize_x, blksize_y, blksize_z, marker_chunk_dim);
    const size_t num = blksize_x * blksize_y * blksize_z;

    SZ3::Config conf;
    const SZ3::uchar* cmp_conf_pos = reinterpret_cast<const SZ3::uchar*>(compressedData);
    conf.load(cmp_conf_pos);
    if (conf.num != num || conf.cmprAlgo != SZ3::ALGO_NOPRED) {
        SZ3::Config libConf(blksize_z, blksize_y, blksize_x);
        libConf.cmprAlgo = SZ3::ALGO_NOPRED;
        libConf.errorBoundMode = SZ3::EB_ABS;
        T* deData = new T[num];
        SZ_decompress<T>(libConf, compressedData, outSize, deData);
        SZ3::HuffmanEncoder<int>::clear_forced_marker();
        return deData;
    }

    const bool prof = std::getenv("DEC_PROF") != nullptr;
    auto t0 = std::chrono::high_resolution_clock::now();
    ...
    auto t1 = std::chrono::high_resolution_clock::now();
    if (std::getenv("DEC_DUMPBUF")) {
        static int dumpId = 0;
        int id;
        #pragma omp critical
        id = dumpId++;
        char path[128];
        snprintf(path, sizeof(path), "/tmp/szbuf_%zu_%d.bin", blksize_x, id);
        FILE* f = fopen(path, "wb");
        if (f) { fwrite(buffer, 1, bufferSize, f); fclose(f); }
    }
    ...
    T* deData = new T[num];
    // Fused decode+recover: write the recovered value during Huffman decode and
    // skip the intermediate quant-index vector. Unpredictable (qi==0) values
    // would need row-major recover order, so the fused pass only counts them
    // (atomically -- chunks may decode in parallel) and falls back to the exact
    // two-pass path in the rare case any exist.
    const double q_eb = quantizer.get_eb();
    const int q_radius = quantizer.get_out_range().second / 2;
    std::atomic<size_t> unpred_count{0};
    const SZ3::uchar* decode_start = buffer_pos;
#if 1  // recover LUT (was DEC_LUT experiment; identical values, ~13% faster decode)
    // precomputed recover table: one indexed load per symbol instead of the
    // int->double multiply chain (values identical by construction)
    std::vector<T> rec_lut(static_cast<size_t>(2 * q_radius) + 1);
    for (size_t q = 0; q < rec_lut.size(); ++q) {
        rec_lut[q] = static_cast<T>(2 * (static_cast<int>(q) - q_radius) * q_eb);
    }
    const T* lut = rec_lut.data();
    bool fused = quant_inds_size == num &&
                 encoder.decode_marked_transform(buffer_pos, quant_inds_size, deData,
                     [&](size_t, int qi) -> T {
                         if (qi == 0) {
                             unpred_count.fetch_add(1, std::memory_order_relaxed);
                             return 0;
                         }
                         return lut[qi];
                     });
    ...
    bool fused = quant_inds_size == num &&
                 encoder.decode_marked_transform(buffer_pos, quant_inds_size, deData,
                     [&](size_t, int qi) -> T {
                         ...
                         return static_cast<T>(2 * (qi - q_radius) * q_eb);
                     });
#endif
    if (!fused || unpred_count.load() != 0) {
        buffer_pos = decode_start;
        std::vector<int> quant_inds = encoder.decode_fast(buffer_pos, quant_inds_size);
        for (size_t i = 0; i < num; ++i) {
            deData[i] = quantizer.recover(0, quant_inds[i]);
        }
    }
    auto t3 = std::chrono::high_resolution_clock::now();
    ...
    if (prof) {
        ...
        fprintf(stderr, "[dec prof %zu^3] zstd %.1f ms, load %.1f ms, huffman %.1f ms, recover %.1f ms (buf %zu B)\n",
                blksize_x, ms(t0, t1), ms(t1, t2), ms(t2, t3), ms(t3, t4), bufferSize);
    }
    SZ3::HuffmanEncoder<int>::clear_forced_marker();
    return deData;
}
                                           ...
                                           size_t& decodedSymbols, size_t& unpredZeros)
{
    ...
    size_t quant_inds_size = 0;
    SZ3::read(quant_inds_size, buffer_pos, remaining_length);
    std::vector<int> quant_inds = encoder.decode_marked_chunks_fast(buffer_pos, quant_inds_size, rowMajorChunks);
    encoder.postprocess_decode();
...
int run_typed(int argc, char* argv[])
{
    ...
    // Even-z planes feed sub-blocks 0..3 only; slicing them first makes the
    // base block available early. Odd-z slicing overlaps the base compress below.
    slice_full_data_parity(full_data, sub_block_data, full_dim_x, full_dim_y, full_dim_z, 0);
    ...
    // Per-level error-bound multipliers (relative to eb).
    //   Original allocation was 1.0 / 2.5 / 6.25.
    //   RD-retuned to 0.7 / 2.7 / 8.2: the high (finest, 7/8 of voxels) level is
    //   predicted so well that its residuals quantize to ~0 and barely affect MSE,
    //   so it can be loosened (6.25->8.2) for a big bit saving; the base/coarse
    //   level dominates MSE, so it is tightened (1.0->0.7) to hold PSNR. Net: same
    //   PSNR, ~+5% CR, no extra compute. Override any of them via env for sweeps.
    const double mult_base = std::getenv("MULT_BASE") ? atof(std::getenv("MULT_BASE")) : 0.7;
    const double mult_low  = std::getenv("MULT_LOW")  ? atof(std::getenv("MULT_LOW"))  : 2.7;
    const double mult_high = std::getenv("MULT_HIGH") ? atof(std::getenv("MULT_HIGH")) : 8.2;
    auto sz_start = std::chrono::high_resolution_clock::now();
    char* tmp = nullptr;
    #pragma omp parallel
    {
        // One thread compresses the base block while the rest slice the odd-z
        // planes into sub-blocks 4..7; the two touch disjoint buffers.
        #pragma omp single nowait
        {
            auto base_c0 = std::chrono::high_resolution_clock::now();
            tmp = SZ_compress4De(low_block_data[0], low_dim_x, low_dim_y, low_dim_z, mult_base * eb, szcompressedSize);
            auto base_c1 = std::chrono::high_resolution_clock::now();
            if (std::getenv("CMP_PROF2")) {
                fprintf(stderr, "[cmp prof] base SZ3 compress %.1f ms\n",
                        std::chrono::duration_cast<std::chrono::microseconds>(base_c1 - base_c0).count() * 1e-3);
            }
        }
        #pragma omp for schedule(dynamic)
        for (int zz = 0; zz < full_dim_z / 2; ++zz) {
            const int z = 2 * zz + 1;
            for (int y = 0; y < full_dim_y; ++y) {
                for (int x = 0; x < full_dim_x; ++x) {
                    const int sub_index = 4 | ((y & 1) << 1) | (x & 1);
                    const int pos = (z / 2) * (full_dim_y / 2 * full_dim_x / 2) + (y / 2) * (full_dim_x / 2) + x / 2;
                    sub_block_data[sub_index][pos] = full_data[(size_t)z * (full_dim_x * full_dim_y) + (size_t)y * full_dim_x + x];
                }
            }
        }
    }
    ...
    // z-slab task list reused for the balanced preprocess and depreprocess phases.
    auto low_zslab_tasks = [&]() {
        constexpr int kZChunk = 16;
        std::vector<std::array<int, 3>> tasks;
        tasks.reserve(7 * ((low_dim_z + kZChunk - 1) / kZChunk));
        for (int block = 0; block < 7; ++block)
            for (int z0 = 0; z0 < low_dim_z; z0 += kZChunk)
                tasks.push_back({block, z0, std::min(z0 + kZChunk, low_dim_z)});
        return tasks;
    }();
    // Phase 1: balanced preprocess.
    #pragma omp parallel for schedule(dynamic)
    for (int t = 0; t < static_cast<int>(low_zslab_tasks.size()); ++t) {
        const int block = low_zslab_tasks[t][0], z0 = low_zslab_tasks[t][1], z1 = low_zslab_tasks[t][2];
        preprocess_block_zslab(block, low_block_data[block + 1], low_block_data[0], low_diff_data[block],
                               low_dim_x, low_dim_y, low_dim_z, z0, z1);
    }
    // Phase 2: SZ-encode (modifies low_diff_data in place to the decompressed diff).
    #pragma omp parallel for reduction(+:allSize)
    for (int block = 0; block < 7; ++block)
    {
        low_comp[block] = SZ_compress(low_diff_data[block], low_dim_x, low_dim_y, low_dim_z,
                                      roi_low_dim, mult_low * eb, low_compressedSize[block]);
        allSize += low_compressedSize[block];
    }
    // Phase 3: balanced depreprocess from the (in-place decompressed) diff.
    #pragma omp parallel for schedule(dynamic)
    for (int t = 0; t < static_cast<int>(low_zslab_tasks.size()); ++t) {
        const int block = low_zslab_tasks[t][0], z0 = low_zslab_tasks[t][1], z1 = low_zslab_tasks[t][2];
        depreprocess_block_zslab(block, low_diff_data[block], low_block_data[0], low_de_sub_block[block],
                                 low_dim_x, low_dim_y, low_dim_z, z0, z1);
    }
    ...
    // The fine-level depreprocess scatters directly into the full-resolution
    // volume (fused merge), so no per-block de_sub buffers are needed.
    char* comp[7] = {};
    T* deData[7] = {};
    ...
    T* reconstructed_full_data = full_validate ? new T[(size_t)full_dim_z * full_dim_y * full_dim_x] : nullptr;

    auto start = std::chrono::high_resolution_clock::now();
    // Fused phase 1+2a: preprocess each (block, z-slab) into a thread-local slab
    // buffer and quantize it right away -- the high-level diff never touches
    // DRAM (the slab fits in cache) and no 7x67MB diff arrays are allocated.
    // Unpred values are collected per slab and concatenated in slab order, so
    // the stream layout stays identical to the unfused path.
    const double eb_high = mult_high * eb;
    const size_t hnum = static_cast<size_t>(dim_z) * dim_y * dim_x;
    std::unique_ptr<int[]> high_q[7];
    for (int b = 0; b < 7; ++b) high_q[b].reset(new int[hnum]);
    {
        constexpr int kZChunk = 16;
        const int nslab = (dim_z + kZChunk - 1) / kZChunk;
        std::vector<std::vector<T>> high_unpred[7];
        for (int b = 0; b < 7; ++b) high_unpred[b].resize(nslab);
        SZ3::Config hconf(dim_z, dim_y, dim_x);
        const int h_radius = hconf.quantbinCnt / 2;
        std::vector<T> dec_lut(static_cast<size_t>(h_radius) + 1);
        for (size_t h = 0; h < dec_lut.size(); ++h) {
            dec_lut[h] = static_cast<T>(2 * static_cast<int>(h) * eb_high);
        }
        std::vector<std::array<int, 3>> tasks;
        tasks.reserve(7 * nslab);
        for (int block = 0; block < 7; ++block)
            for (int z0 = 0; z0 < dim_z; z0 += kZChunk)
                tasks.push_back({block, z0, std::min(z0 + kZChunk, dim_z)});
        const size_t dim_xy_h = static_cast<size_t>(dim_x) * dim_y;
        std::vector<std::vector<T>> slab_buf(omp_get_max_threads());
        #pragma omp parallel for schedule(dynamic)
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            const int block = tasks[t][0], z0 = tasks[t][1], z1 = tasks[t][2];
            std::vector<T>& sb = slab_buf[omp_get_thread_num()];
            if (sb.size() < static_cast<size_t>(kZChunk) * dim_xy_h) {
                sb.resize(static_cast<size_t>(kZChunk) * dim_xy_h);
            }
            // kernels index the output with the global idx; shift the base so
            // the slab lands at the start of the scratch buffer
            T* slab_out = sb.data() - static_cast<size_t>(z0) * dim_xy_h;
            preprocess_block_zslab(block, sub_block_data[block + 1], reconstructed_sub_0, slab_out,
                                   dim_x, dim_y, dim_z, z0, z1);
            const size_t i0 = static_cast<size_t>(z0) * dim_xy_h;
            const size_t n = static_cast<size_t>(z1 - z0) * dim_xy_h;
            quantize_range_nodec(sb.data(), n, eb_high, h_radius, dec_lut.data(),
                                 high_q[block].get() + i0, high_unpred[block][z0 / kZChunk]);
        }
        // Phase 2b: tree + marked encode + zstd per stream.
        #pragma omp parallel for reduction(+:allSize)
        for (int block = 0; block < 7; ++block)
        {
            std::vector<T> unpred_all;
            for (auto& u : high_unpred[block]) {
                unpred_all.insert(unpred_all.end(), u.begin(), u.end());
            }
            comp[block] = SZ_compress_finish<T>(high_q[block].get(), hnum, unpred_all,
                                                dim_x, dim_y, dim_z, roi_high_dim,
                                                eb_high, compressedSize[block]);
            allSize += compressedSize[block];
        }
        for (int b = 0; b < 7; ++b) high_q[b].reset();
    }
    double original_size = (double)full_dim_z * full_dim_y * full_dim_x * sizeof(T);
    double CR = original_size / (double)allSize;
    std::cout << "CR: " << CR << std::endl;
    if (std::getenv("CMP_PROF")) {
        size_t lowSum = 0, highSum = 0;
        for (int b = 0; b < 7; ++b) { lowSum += low_compressedSize[b]; highSum += compressedSize[b]; }
        fprintf(stderr, "[cmp prof] base %zu B, low x7 %zu B, high x7 %zu B, total %zu B\n",
                szcompressedSize, lowSum, highSum, allSize);
        for (int b = 0; b < 7; ++b)
            fprintf(stderr, "[cmp prof] high block %d: %zu B\n", b, compressedSize[b]);
    }
    ...
    if (full_validate) {
    // With few threads, 7-way block parallelism wins (no per-block serial
    // zstd/table-build on the critical path); with many threads the 7-way
    // limit dominates, so decode blocks serially and let each stream's marked
    // chunks decode in parallel inside decode_marked_transform.
    const bool chunk_parallel_decode = omp_get_max_threads() > 8;
    auto low_decompress_start = std::chrono::high_resolution_clock::now();
    if (chunk_parallel_decode) {
        for (int block = 0; block < 7; ++block)
        {
    ...
    //           << low_time_taken_decompress_sz << " sec" << std::endl;
    {
        ...
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            const int block = tasks[t][0], z0 = tasks[t][1], z1 = tasks[t][2];
            depreprocess_block_zslab(block, low_deData[block], decompressed_data, low_de_sub_block[block],
                                     low_dim_x, low_dim_y, low_dim_z, z0, z1);
        }
    }
    ...
    // Flatten (block, z-slab) into one dynamically-scheduled loop. The 7 blocks
    // are very imbalanced (block 6 alone ~= the other six combined), so a plain
    // parallel-for over blocks leaves cores idle on the block-6 tail. Each output
    // z-plane only reads the reference, so z-slabs are independent.
    // The reconstructed values are scattered directly into the full volume
    // (fused merge); the reference sub-block 0 is scattered alongside as an
    // eighth task stream (block id -1).
    {
        constexpr int kZChunk = 16;
        std::vector<std::array<int, 3>> tasks;
        tasks.reserve(8 * ((dim_z + kZChunk - 1) / kZChunk));
        for (int block = -1; block < 7; ++block)
            for (int z0 = 0; z0 < dim_z; z0 += kZChunk)
                tasks.push_back({block, z0, std::min(z0 + kZChunk, dim_z)});
        const size_t fdx = 2 * (size_t)dim_x;
        const size_t fdxy = fdx * 2 * (size_t)dim_y;
        #pragma omp parallel for schedule(dynamic)
        for (int t = 0; t < static_cast<int>(tasks.size()); ++t) {
            const int block = tasks[t][0], z0 = tasks[t][1], z1 = tasks[t][2];
            if (block < 0) {
                for (int z = z0; z < z1; ++z) {
                    for (int y = 0; y < dim_y; ++y) {
                        const T* src = reconstructed_sub_0 + ((size_t)z * dim_y + y) * dim_x;
                        T* frow = reconstructed_full_data + 2 * (size_t)z * fdxy + 2 * (size_t)y * fdx;
                        for (int x = 0; x < dim_x; ++x) {
                            frow[2 * (size_t)x] = src[x];
                        }
                    }
                }
            } else {
                depreprocess_block_zslab_to_full(block, deData[block], reconstructed_sub_0,
                                                 reconstructed_full_data, dim_x, dim_y, dim_z, z0, z1);
            }
        }
    }
    ...
    if (full_validate) {
        ...
        for (int z = query.z.begin; z <= query.z.end; ++z) {
            for (int y = query.y.begin; y <= query.y.end; ++y) {
                for (int x = query.x.begin; x <= query.x.end; ++x) {
                    // fused merge: the full-decode reference values live in
                    // reconstructed_full_data directly
                    const T full_value = reconstructed_full_data[
                        ((size_t)z * full_dim_y + y) * full_dim_x + x];
    ...
    // The full volume is already assembled during the fused depreprocess pass,
    // so there is no separate merge step.
...
{
    // Flush denormals to zero: the dynamic inverse-variance weights
    // underflow to denormals on flat/zero regions (e.g. cloud fields),
    // which trip the slow x86 microcode path. Values are ~1e-39, far below
    // the error bound, so flushing them is numerically negligible.
    _MM_SET_FLUSH_ZERO_MODE(_MM_FLUSH_ZERO_ON);
    _MM_SET_DENORMALS_ZERO_MODE(_MM_DENORMALS_ZERO_ON);
    // zstd level follows the SZ3 default (3). Export SZ3_ZSTD_LEVEL to trade
    // compress speed for CR (9: +4% CR at ~2% cost; 19: +7% CR at 2.2x cost).
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0]
                  << " <error_bound> <raw_file> <dim_x> [dim_y dim_z] <-f|-d> [--chunks high low] [--query-only] [x0 y0 z0 x1 y1 z1]"
                  << std::endl;
