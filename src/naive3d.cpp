                                      ...
                                      int dim_x, int dim_y, int dim_z, int dim_xy)
{
    switch (block) {
        case 2:
            return 0.5f * (interp_axis(ref_x, 1, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy) +
                           interp_axis(ref_y, 0, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy));
        case 4:
            return 0.5f * (interp_axis(ref_x, 2, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy) +
                           interp_axis(ref_z, 0, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy));
        case 5:
            return 0.5f * (interp_axis(ref_y, 2, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy) +
                           interp_axis(ref_z, 1, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy));
        case 6:
            return (interp_axis(ref_xy, 2, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy) +
                    interp_axis(ref_xz, 1, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy) +
                    interp_axis(ref_yz, 0, idx, x, y, z, dim_x, dim_y, dim_z, dim_xy)) / 3.0f;
        default:
            return 0.0f;
    }
}
        ...
        for (int y = 0; y < dim_y; ++y)
        {
            for (int x = 0; x < dim_x; ++x)
            {
                ...
                switch(block)
                {
                    ...
                    case 2: // "011" - Full 16-point bicubic in Y and X
                        ...
                        else
                        {
                            ...
                            for (int dv = -1; dv <= 2; ++dv) {
                                const int base_idx = z * dim_xy + (y + dv) * dim_x + x;
                                const float pred_x = W[0] * ref[base_idx - 1] + W[1] * ref[base_idx] +
                    ...
                    case 4: // "101" - Full 16-point bicubic in Z and X
                        ...
                        else
                        {
                            ...
                            for (int dw = -1; dw <= 2; ++dw) {
                                const int base_idx = (z + dw) * dim_xy + y * dim_x + x;
                                const float pred_x = W[0] * ref[base_idx - 1] + W[1] * ref[base_idx] +
                                                     W[2] * ref[base_idx + 1] + W[3] * ref[base_idx + 2];
                    ...
                    case 5: // "110" - Full 16-point bicubic in Z and Y
                        ...
                        else
                        {
                            ...
                            for (int dw = -1; dw <= 2; ++dw) {
                                float pred_y = 0.0f;
                                for (int dv = -1; dv <= 2; ++dv) {
                                    const int base_idx = (z + dw) * dim_xy + (y + dv) * dim_x + x;
